...
if ($LASTEXITCODE -eq 0) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "docker run -p 8080:8080 -e SPRING_PROFILES_ACTIVE='prod' -e DATASOURCE_URL='tu-url' -e DATASOURCE_USERNAME='tu-usuario' -e DATASOURCE_PASSWORD='tu-password' paxtech-web-services"
        Write-Host ""
